...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/05/20 13:16:09 by urlooved &&       #+#    #+#             */
/*   Updated: 2025/05/20 13:16:11 by urlooved &&      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	*t_path = ft_strtrim(line + 2, " \t");
	ft_lstadd_back(&game->garbage_coll, ft_lstnew(*t_path));
}

/*
This function checks if a texture has already been set. 
For exemples in the file 
	we found more than 1 time the assignation of a texture to a N,W,S,E
If the texture is already set, 
it marks the map data as having a duplicate texture.
*/
static bool	is_texture_duplicated(char *cardinal_texture, t_map *d_map)
{
	if (cardinal_texture)
	{
		d_map->has_duplicated_texture = true;
		return (true);
	}
	return (false);
}
...
/*
	This function processes a line of input to extract
	and store color data for the map.
	It checks the prefix of the line to determine
	the type of data and handles it accordingly.
*/
static void	handle_colors(t_game *game, t_map *d_map, char *line)
{
	if (!ft_strncmp((const char *) line, "C ", 2))
		handler_init_parse_colors(game, line, &d_map->ceiling_color);
...
static void	handle_path_textures(t_game *game, t_map *d_map, char *line)
{
	if (!ft_strncmp((const char *) line, "NO ", 3)
		&& !is_texture_duplicated(d_map->n_texture, game->map_data))
		init_texture_path_from_line(&d_map->n_texture, game, line);
	else if (!ft_strncmp((const char *) line, "SO ", 3)
		&& !is_texture_duplicated(d_map->s_texture, game->map_data))
		init_texture_path_from_line(&d_map->s_texture, game, line);
	else if (!ft_strncmp((const char *) line, "WE ", 3)
		&& !is_texture_duplicated(d_map->w_texture, game->map_data))
		init_texture_path_from_line(&d_map->w_texture, game, line);
	else if (!ft_strncmp((const char *) line, "EA ", 3)
		&& !is_texture_duplicated(d_map->e_texture, game->map_data))
...
void	handler_colors_and_textures(t_game *game, t_map *d_map)
{
	...
	while (game->file_raw_data[i])
	{
		...
		if (game->map_data->has_duplicated_texture == true)
			err_msg_free_gc_exit("HC&T: T. are duplicated", game->garbage_coll);
		i++;
	}
	...
	if (d_map->ceiling_color.is_inited == false
		|| d_map->floor_color.is_inited == false)
