...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/05/20 13:16:26 by urlooved &&       #+#    #+#             */
/*   Updated: 2025/05/20 13:16:28 by urlooved &&      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	init_map_data(int start, t_game *game)
{
	...
	map = malloc_gc(sizeof(char *) * (get_map_amount_rows(start, game) + 1),
			game->garbage_coll);
	i = 0;
...
bool	handler_map(t_game *game)
{
	...
	if (!has_map_one_player(game->map_data->map))
		return (false);
	if (!has_map_valide_atributes(game->map_data->map))
		return (false);
	if (!is_map_pos_context_correct(game->map_data->map))
		return (false);
	return (true);
}
