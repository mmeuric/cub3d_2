...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/05/20 13:16:49 by urlooved &&       #+#    #+#             */
/*   Updated: 2025/05/20 13:16:51 by urlooved &&      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

static void	init_start_direction(t_game *game, int i, int j)
{
	...
	while (game->map_data->map[i])
	{
		...
		while (game->map_data->map[i][j])
		{
			if (is_cardinal_player(game->map_data->map[i][j]))
				init_start_direction(game, i, j);
			j++;
		}
		i++;
	}
	game->map_data->player.cur_plane_x = (0.66
			* (-1 * game->map_data->player.next_coor_y));
	game->map_data->player.cur_plane_y = (0.66
