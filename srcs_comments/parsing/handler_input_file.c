...
// Will scrap from the inputed file and create a copy into char** game->file_raw_data.
...
/* 
	handler_input_file will: 
		- init data to null, 
		- extract raw_data:  from the inputed file to game->(char **)file_raw_data, 
...
void	handler_input_file(t_game *game, char **av)
{
	...
	extract_file_raw_data(game, av[1]);										// Will extract the data from the inputed file to game->(char **)file_raw_data
    
	handler_pre_clean_file(game);                                 			// Pre-clean texture and color lines if necesary 
	//print_file_raw_data(game); 											//* debug 

	handler_colors_and_textures(game, game->map_data); 						// Will init / parse textures & colors

	if (!handler_map(game))													// Will study the map attributes and the souroundings of each position.
		err_msg_free_gc_exit("HM : Error parsing", game->garbage_coll);				// free error in case init_map fails
}
