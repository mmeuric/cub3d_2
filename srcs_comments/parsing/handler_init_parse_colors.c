...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/05/14 12:19:03 by urlooved &&       #+#    #+#             */
...
void handler_init_parse_colors(t_game *game, char *line, t_color *rgb)
{
	...
	if (rgb && rgb->is_inited == true)
		err_msg_free_gc_exit("HIPC : parser error", game->garbage_coll);					//* possible error : colors duplicated
	...
	if (!has_valid_rgb_data(color_components))
	{
		...
		err_msg_free_gc_exit("HIPC : parser error", game->garbage_coll);						//* possible error : err color data
	}
	...
	if (!init_output_color(color, color_components))
		err_msg_free_gc_exit("HIPC : parser error", game->garbage_coll);					//* possible error : err color out band
	free_array_strings(color_components);
	*rgb = *color;
}