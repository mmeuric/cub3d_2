...
void	paint_wall_segment(t_game *game, int x, int y, int z)
{
	...
	while (i < z)
	{
		game->cal->texture_y = (int)game->cal->texture_coor & (TEXTURE_H - 1);
...
// Fills the buffer with ceiling or floor color based on the 'j' flag (0 = ceiling, 1 = floor).
void	render_ceiling_floor(t_game *game, int x, int j)
{
	...
	if (j == 0)
	{
		...
		{
			game->mlx_data->buf[i][x] = game->map_data->ceiling_color.output_color;
			game->mlx_data->flag_buf = true;
			i++;
		}
	}
